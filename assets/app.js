...
const WAREKI = {
  2019: '令和1', 2020: '令和2', 2021: '令和3', 2022: '令和4', 2023: '令和5',
...
// 乗務後点呼のフィールドプレフィックス定義
// g3=乗務後①, g5=乗務後②, g7=乗務後③
// ※乗務前点呼は常に g1 を使用（最初の乗務前）
const END_PREFIXES = ['g3', 'g5', 'g7']; // 昇順（①→②→③）
const START_PREFIX = 'g1';

/**
 * entry から「最後の乗務後点呼」プレフィックスを返す
 * 時間・方法いずれかが入力されている最も後ろのものを採用
 */
function getLastEndPrefix(entry) {
  // 後ろから順に探す（g7→g5→g3）
  for (const p of [...END_PREFIXES].reverse()) {
    if (entry[p + 'Time'] || entry[p + 'Method']) return p;
  }
  return null;
}
...
async function buildList() {
  ...
  for (const [ymd, day] of Object.entries(dailyData).sort()) {
    ...
    for (const e of day.entries) {
      ...
      // 最後の乗務後点呼を動的に取得
      const lastEndPfx = getLastEndPrefix(e);
      ...
      rows.push({
        ...
        driver: e.driver,
        dest: e.dest,
        // 乗務前点呼：常に g1（最初の乗務前）の時間
        startTime: e[START_PREFIX + 'Time'] || '',
        startMethod: e[START_PREFIX + 'Method'] || '',
        // 乗務後点呼：最後に入力されているもの（g3/g5/g7 のうち後ろから最初に値がある）
        endTime: lastEndPfx ? (e[lastEndPfx + 'Time'] || '') : '',
        endMethod: lastEndPfx ? (e[lastEndPfx + 'Method'] || '') : '',
        // どの乗務後フィールドを使ったか（デバッグ・参考用）
        endSource: lastEndPfx || '',
        supervisor: day.supervisor,
        manager: day.manager,
        manager2: day.manager2,
        helper: day.helper,
        helper2: day.helper2,
      });
    }
  ...
  let html = `<div class="tbl-wrap"><table>
    <thead><tr>
      <th>日付</th><th>車番</th><th>運転者</th><th>行き先</th>
      <th>乗務前点呼時間</th>
      <th>乗務後点呼時間</th>
      <th>統括管理者</th><th>運行管理者</th><th>運行管理者②</th><th>補助者</th><th>補助者②</th>
    </tr></thead><tbody>`;
  ...
  for (const r of rows) {
    html += `<tr>
      ...
      <td>${r.driver || '—'}</td>
      <td>${r.dest || '—'}</td>
      <td class="td-mono">${timeCell(r.startTime)}</td>
      <td class="td-mono">${timeCell(r.endTime)}</td>
      <td>${r.supervisor || '—'}</td>
      <td>${r.manager || '—'}</td>
      <td>${r.manager2 || '—'}</td>
      <td>${r.helper || '—'}</td>
